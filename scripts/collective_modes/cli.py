...
def _case_arguments(parser: argparse.ArgumentParser, cylindrical: bool = False) -> None:
    ...
    parser.add_argument("--axis-xy", nargs=2, type=float); parser.add_argument("--rcnt-A", type=float)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = sub.add_parser("fit-current", help="per-(n,m) DHO parameter table for Phi_J"); p.add_argument("--current-csv", required=True, type=Path); p.add_argument("--output", required=True, type=Path); p.add_argument("--channel", default="L"); p.add_argument("--column", default="CJJ_normalized"); p.add_argument("--fit-min-ps", type=float, default=0.0); p.add_argument("--fit-max-ps", type=float, required=True); p.set_defaults(func=commands.fit_current)
